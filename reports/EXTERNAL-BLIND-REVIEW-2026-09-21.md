...
**R4 余额（2026-09-23 三批后）**：**0 项（R4 归零）**。
已完成 —— §2.1 generate_contract 三项（`015f022`）· §2.2 随 skill-sync 归档关闭（`7032fb0`）·
§2.3 S3 `k8s_helper` 状态掩盖 · S1 `index-project` venv 探测 · S4 `idea-mcp` SSE 快速失败（`b19049c`）·
S2 `k8s-logs` 通道一致性（用户裁定：以 WSL 原生 `kubectl` 为准，缺失/未配置时**请求用户授权安装配置**，
不静默回退 `cmd.exe`；SKILL.md 时间点快照改为探测式）。
**本轮顺带新发现（未在本 8 项内）** —— `path-audit.py` `PATH_RE` 无词边界 → `kubeconfig/连接错误时`、
`archived/skills/skill-sync/…` 的中间段被当引用（一轮内 2 次实测误报）；建议一行负向后顾修复，待确认。

原 skills 脚本组 4 项 —— `k8s_helper` `.status.phase` 掩盖 CrashLoopBackOff ·
