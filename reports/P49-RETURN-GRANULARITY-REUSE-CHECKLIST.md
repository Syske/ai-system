...
| B | A + quality-gates 自动化（实现层复用 Gate） | 更硬；**2026-09-09 补调研**：结构相似类（改名重写）可自动化（dupehound 结构指纹 / Sonar CPD），语义类（Type-4）仍高误报；转后续待办，dupehound 为候选调研对象 |
| C | 仅加 review 条目（不动 implement 清单） | 缺实现期自查，拦截点后移 |

## 4. Recommendation

**方案 A**。理由：① 两条实证缺陷恰好落在「自查清单无此项」的空隙，清单补齐即拦截；② 三落点（计划/实现/review）让 AI 先自检、review 兜底，符合「AI 先自检、用户后决策」；③ 自动化检测**按返工类区分**（2026-09-09 补调研）：结构相似类（AI copy/参考重写后改标识符）已有可自动化工件（dupehound 结构指纹 / Sonar CPD），语义等价但结构不同类（T-025/026 自研替代既有）仍不可靠——B 项转后续待办（quality-gates，dupehound 为候选），不在本提案范围。
...
> **后续调研登记（2026-09-09，用户确认）**：quality-gates 自动化待办的调研对象 = **dupehound**（Rust 开源，结构指纹
> 式重复函数检测，专为 AI 写码为主的仓设计；改名/改字面量后仍可检出；离线、CI gate、无 AI 依赖）。验证三步：
