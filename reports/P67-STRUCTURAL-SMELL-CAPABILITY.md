...
| 指标 | 本提案立场 |
|---|---|
| 方法长度 | ✅ **已裁决（2026-09-23，`MAINTENANCE-2026-08-06` 按 P1′ 实施）**：单一刻度 **target ≤40 / hard ceiling 80**（`clean-code.md` 为唯一来源；编排/监听/定时/Controller 入口**不豁免**）。带门禁语义：**>80 → FAIL**；接近 40 → WARN。原「≤80 统一」方案未采纳（会把标准放宽 2 倍，且需「哪类方法算编排」的启发式分类 → 歧义） |
| 嵌套深度 | 沿用 **≤3 层**（`clean-code.md:68`） |
| 圈复杂度 | 沿用 **≤20**（checkstyle），但把「**劣化**」定为阻断项（升幅超阈值即拦），而非现在的「仅 warn」 |
| 类大小 / 方法数 | **仍待裁决**（仓内空白）：建议类 ≤500 行、方法数 ≤10（God Object 信号）—— 落点同上（`clean-code.md` 单一来源），可与本提案 §4.2 的真实触发一并裁决 |
