...
| `<workspace>/metrics/` (workspace level, outside the repo) | Health metrics snapshots | `quick-check-{date}.json` (quick-check) + `maintain-{date}.json` (repo-metrics) + `prompt-{date}.json` | Metric data storage |
...
- Generated outputs → workspace-level `logs/` + `metrics/` (machine-local, outside every repo) and ai-system `reports/`; never commit temp/cache.
