...
| `comment-lint.py` | 注释质量检查（P69 MVP）：只针对**本次改动新增的 Java 注释**判「确定可删 / 需裁定 / 保留」，策略（六级分类 + 稳定规则 id `CQ-*` + 判定顺序 + 安全原则「宁可漏删不可误删」）见 `governance/standards/common/documentation.md` → Comment Content。**当前阶段 S5：候选提取 + diff 限定 + 规则判定 + 安全修复**（候选字段 file/行列/字符偏移/文本/kind/缩进/前后代码/所属方法/所属类；JavaDoc 标记 skip；`--diff`/`--changed` 只纳入 `git diff -U0 HEAD` 新增行内的注释 —— 改动文件取 `git status --porcelain`（含未跟踪、rename 取新路径），未跟踪/无 HEAD/非 git 仓 → 整文件视为新增；非 git 仓退化为全量并告警）。双通道输出必须一致：`tree-sitter-language-pack` 提供语法（**可选加速依赖**，缺失时探测式降级到标准库词法状态机）。**与既有检查分工不重复**：单行块 `/** xxx */` 与注释内任务编号属 `format-check.py`，注释语言属 `repo-lint.py`。输出 `KEEP`/`DELETE`/`REVIEW` 三档及规则 id（六类：`CQ-MEANINGFUL` 白名单优先命中 · `CQ-SECTION-HEADER` · `CQ-AI-NOISE` · `CQ-OBVIOUS` · `CQ-DUPLICATE` · `CQ-UNCERTAIN`）；exit **0=PASS（无 DELETE/REVIEW）· 1=WARN（有需裁定 REVIEW）· 2=FAIL（有确定可删 DELETE）· 3=用法/IO/环境错误**。安全护栏：白名单优先于一切删除类 · OBVIOUS 需「开头动词 + 与下一行代码链接」双命中 · 字段/枚举常量上的注释不得判复述（名字直译是「需改」不是「可删」）· 动词不在开头的名词短语/叙述句不删。用法：`python3 tools/comment-lint.py [check] <path> [--diff\|--changed] [--parser …] [--dump-candidates] [--json] [--report-only]` · `python3 tools/comment-lint.py fix <path> [--diff] [--dry-run\|--apply] [--json]`。**fix 默认只做 dry-run 预演（打印 unified diff，不写盘）**，`--apply` 才写回且只删确定类；行级重写（独立行删整行 / 行尾只剔注释），幂等可重跑。`--report-only` 退出码封顶为 1（供 MVP 阶段门禁只报不拦）。单测 `cli/tests/test_comment_lint.py`（43 项） |
| `runtime_state.py` | **运行时态路径单一来源**（`LOG_DIR` / `METRICS_DIR`）——两者一律**位于工作区层、仓库之外**（2026-09-24 事故：仓库内且被 gitignore 的 `ai-system/logs/` 被一条 `git clean -fdx` 清空约 200 份记录）；被 maintain-delta / maintain-report / prompt-metrics / quick-check / setup 共用 |
