...
> → Operating Rules (constrain behaviour) → Standards (define quality)
...
|---|---|---|
| `README.md` | Entry / orientation (structure, quick commands) | Manual |
| `OPERATIONS.md` | Operations manual — entry flows, lint/check usage, menu maintenance, workflow rules | Manual |
| `README_MIGRATION.md` | Migration package checklist (shipped inside a packed copy) | **Generated by `tools/pack.py`** — do not edit directly |

Workflow chain / selection table: single source is `workflows/README.md` (OPERATIONS §1.1 and this README reference it, they do not re-maintain it).
...
├── config/       Menu, workflow-registry, environment and i18n configuration
...
├── skills/       Agent skills (implementation behaviour, decision logic)
├── templates/    Runtime / prompt / asset templates
├── tools/        repo-lint, check, pack, setup and other maintenance scripts
├── workflows/    Workflow entry contracts (README with selection table)
└── reports/      Generated analysis and maintenance reports
```
...
3. **Run `python tools/repo-lint.py --repo-root .`** and **`python tools/check.py`**
   before proposing structural changes. Fix all BLOCKER and ERROR items.
...
# System integrity gate — run after any modification
...
| `rfc/ADR-0001-*.md` through `ADR-0007-*.md` | Architecture Decision Records |
