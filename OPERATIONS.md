...
* First-time change set scaffold is driven by the methodology provider during spec (/opsx-propose or openspec-cn new change); the chosen kebab-case name becomes the Change ID. Run it inside workspaces/{project_id}/ — openspec-cn locates the project by working directory.
