...
2. 不要赞美、不要复述摘要、不要"总结本文档内容"。只给发现（findings）——按
   `governance/standards/common/output-discipline.md` 的输出纪律执行（可压缩：已知内容/产物/工具原始输出/
   旁白/填充/重复总结；**禁压缩：结论/证据/约束/风险/验证结果/不确定性/裁决请求**）。
