...
**输入**：`/aic-propose` 之后的参数是变更名称（kebab-case），或用户想要构建内容的描述。

**步骤**

0. **确认项目上下文（护栏）**

   openspec-cn 以当前工作目录定位项目：变更将创建在 `./openspec/changes/<name>/`。

   检查：
   - 当前目录必须是目标项目工作区 `workspaces/{project_id}/`（其下存在 `openspec/` 目录）
   - 目录不对 → 使用 **AskUserQuestion tool** 询问目标项目，切换到 `workspaces/{project_id}/` 后再继续
   - `openspec/` 不存在 → 提示先运行 `openspec-cn init`，不要在错误位置创建变更

   上下文加载（最小加载原则）：
   - 若存在本变更的 Preparation Report（prepare 阶段产物），生成 proposal.md 前必读
   - 正式需求建议走主链（prepare → spec，由 provider 在 spec 阶段驱动本命令）；独立使用本命令仅适用于小型、低风险变更
